...
**Music sources (revised 2026-07-29).** Both gift players take a song from exactly two places: a YouTube link (`youtube_url`, rendered as a `youtube-nocookie` embed) or an mp3 the sender uploads (`song_file`, rendered as `.gift-audio` — native controls on a pill-shaped `--gift-card` surface). The Spotify embed and its `spotify_url` field are gone. Uploads go through `uploadAsset` in `lib/supabase/storage.ts`, which sets `contentType` from the file's own type and falls back to an extension map — an mp3 reports `audio/mpeg`, and some browsers report nothing at all, so neither may be assumed to be an image. Audio uploads cap at 12 MB.

**Puzzle-box template (added 2026-07-29).** A second gift player (`/g/puzzle-box`) with a five-stage macrostructure — Delivery → Fall → Puzzle → Reveal → Letter — drawn entirely from the existing `--gift-*` tokens, so it themes with the same four pastels. Stages: a delivery truck drives across a dashed road under a wordmark (loading); the box drops from above onto three blurred `--gift-soft` smoke puffs, and a paper tag carrying the sender's picture swings into place; the tag opens a 3×2 board (one cell pre-placed, five draggable pieces at `scale 0.55` in a tray below) — pieces are `<button>`s, so a tap or Enter places them and drag is an enhancement, never the only path; solving pops the lid, throws seven newspaper clippings outward, and raises the sender's image/GIF (mascot when empty); the letter then reuses `GiftLetter`. Recorded one-shots live in `/public/sfx` (`right`, `wrong`, `paper`) and route through `giftSound`, so the existing mute toggle governs them; box thud and truck whoosh stay synthesised. A new letter-paper variant `.gift-sheet--crumple` fakes creases with four low-alpha diagonal gradients plus an inset shadow. Every stage timeline is skipped under `prefers-reduced-motion`. **Ending:** the last two stages both carry a restart control returning the player to the loading stage.

**Envelope + 3-face letter (added 2026-07-09).** The yes-no player gains an envelope stage (GSAP squash-bounce idle; tap 1 opens the flap, tap 2 pulls the letter) and a 3-face letter (front cover → main with typewriter body, stamp, stickers → back with spinning vinyl + a walking lyric highlight; close folds it back into the envelope, re-openable). Envelope colour tokens (`--env-base` / `--env-deep`, selected via `data-env` in `globals.css`): `cream` `oklch(0.93 0.02 85)`/`oklch(0.87 0.03 82)` · `rose` `oklch(0.90 0.045 15)`/`oklch(0.84 0.06 15)` · `kraft` `oklch(0.85 0.045 75)`/`oklch(0.78 0.055 72)` · `sky` `oklch(0.90 0.04 230)`/`oklch(0.83 0.05 235)`. Letter back panel: `oklch(0.25 0.015 60)` with light text `oklch(0.92–0.93 0.012–0.015 80)`. Stamp ink: `--color-danger`. GSAP (`gsap` + `@gsap/react`) is the animation layer for these gift surfaces.
